...
SELECT V_QUAL_CONFIG_ITSM_SERVICE.MIS_5_NS AS BUSINESS_CLUSTER,
  ...
  V_PROBLEM_ALL_DATA.PRIORITY,
  V_PROBLEM_ALL_DATA.SUMMARY,
  TO_CHAR(V_PROBLEM_ALL_DATA.LAST_MODIFIED_DATE,'DD-MM-YYYY HH24:MI:SS') AS LAST_MODIFIED_DATE,
  V_PROBLEM_ALL_DATA.status,
  CASE
    WHEN V_PROBLEM_ALL_DATA.PRIORITY                                                              = 'Critical'
    AND TRUNC(V_PROBLEM_ALL_DATA.LAST_MODIFIED_DATE) - TRUNC(V_PROBLEM_ALL_DATA.SUBMIT_DATE) + 1 <= 15
    THEN 'passed'
    WHEN V_PROBLEM_ALL_DATA.PRIORITY                                                              = 'High'
    AND TRUNC(V_PROBLEM_ALL_DATA.LAST_MODIFIED_DATE) - TRUNC(V_PROBLEM_ALL_DATA.SUBMIT_DATE) + 1 <= 30
    THEN 'passed'
    WHEN V_PROBLEM_ALL_DATA.PRIORITY                                                              = 'Low'
    AND TRUNC(V_PROBLEM_ALL_DATA.LAST_MODIFIED_DATE) - TRUNC(V_PROBLEM_ALL_DATA.SUBMIT_DATE) + 1 <= 90
    THEN 'passed'
    ELSE 'missed'
  END AS INVESTIGATION_TIME_ACHIEVED,
  V_PROBLEM_ALL_DATA.COORDINATOR_GROUP,
  V_PROBLEM_ALL_DATA.ASSIGNED_GROUP,
  TO_CHAR(V_PROBLEM_ALL_DATA.SUBMIT_DATE,'DD-MM-YYYY HH24:MI:SS') AS SUBMIT_DATE
FROM V_PROBLEM_ALL_DATA
LEFT JOIN V_QUAL_CONFIG_ITSM_SERVICE
ON V_PROBLEM_ALL_DATA."SERVICE"          = V_QUAL_CONFIG_ITSM_SERVICE."ITSM_SERVICE"
WHERE V_PROBLEM_ALL_DATA.STATUS         IN ('Draft','Under Review','Request For Authorization','Assigned','Under Investigation','Pending','Rejected')
AND V_QUAL_CONFIG_ITSM_SERVICE.MIS_3_NS  = 'CAEE II'
AND V_QUAL_CONFIG_ITSM_SERVICE.MIS_5_NS <> 'VM0'
AND V_PROBLEM_ALL_DATA.priority          = 'Critical'
AND V_PROBLEM_ALL_DATA.SUBMIT_DATE      >= sysdate-3 ; 
