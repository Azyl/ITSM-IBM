...
IS
   ...
   IS
      a   T_TABLE_OVER;
   BEGIN
      FOR I IN (SELECT DISTINCT BUSINESS_CLUSTER, CLOSED_DATE
                  FROM MV_ORGANIZATION)
      LOOP
   ...
   PROCEDURE KSL_2 (P_BUSINESS_CLUSTER    VARCHAR2,
                    P_SERVICE             VARCHAR2,
   ...
   BEGIN
        SELECT                                                     --CONTRACT,
               ...
               COUNT (*) AS SERVICED,
                    ROUND (
                         DECODE (SUM (SOLUTION_TIME_ACHIEVED_FLAG),
                                 0, COUNT (*),
          ...
          INTO MISSED_CNT,
               ...
               PROCENT
          FROM V_KSL_2_11
         WHERE STATUS = 'Closed' AND COC_STATUS = 'Current'
      GROUP BY CONTRACT,
               BUSINESS_CLUSTER,
               SERVICE,
               ...
               INCIDENT_TYPE,
               PRIORITY
        HAVING     BUSINESS_CLUSTER = P_BUSINESS_CLUSTER
               AND SERVICE = P_SERVICE
               AND TO_CHAR ("CLOSED_DATE", 'YYYY_MM') = P_CLOSED_DATE
   ...
   FUNCTION KSL_11 (P_BUSINESS_CLUSTER    VARCHAR2,
                    P_SERVICE             VARCHAR2,
                    P_CLOSED_DATE         VARCHAR2)
      RETURN NUMBER
   IS
   ...
   BEGIN
        SELECT COUNT (*)
         ...
         WHERE     INCIDENT_TYPE IN
               ...
               AND (    TRUNC (SUBMIT_DATE) <
                           TO_DATE (P_CLOSED_DATE, 'yyyy_mm')
                    AND (   NOT STATUS IN ('Closed', 'Cancelled')
                         OR (    STATUS = ('Closed')
                             AND TRUNC (CLOSED_DATE) >
                                    LAST_DAY (
                                       TO_DATE (P_CLOSED_DATE, 'yyyy_mm')))))
      ...
      GROUP BY CONTRACT,
               ...
               SERVICE,
               TO_CHAR ("CLOSED_DATE", 'YYYY_MM')
        HAVING     CONTRACT = 'CAEE II'
               AND BUSINESS_CLUSTER = P_BUSINESS_CLUSTER
               AND SERVICE = P_SERVICE;
   ...
   FUNCTION KSL_11_T (P_BUSINESS_CLUSTER VARCHAR2, P_CLOSED_DATE VARCHAR2)
      RETURN T_TABLE
      PIPELINED
   IS
      rec   T_COL;
        ...
        SELECT q.contract,
               ...
               'All' AS priority,
                 ...
                 * (SELECT COUNT (DISTINCT service)
                     ...
                     WHERE     m.contract = q.contract
                           AND m.business_cluster = q.business_cluster)
               - SUM (passed)
                  AS missed,
               ...
               * (SELECT COUNT (DISTINCT service)
                  ...
                  AS serviced,
                    ROUND (
                         DECODE (SUM (passed), 0, COUNT (*), SUM (passed))
                       / (  11
                          * (SELECT COUNT (DISTINCT service)
                               FROM mv_organization m
                              WHERE     m.contract = q.contract
          ...
          FROM (SELECT DATA.*,
                       ...
                       KSL.HT,
                       CASE
                          WHEN RTRIM (DATA."%", '% ') >= KSL.HT THEN 1
                          ELSE 0
                       END
                          AS PASSED,
                       CASE
                          WHEN     KSL.K NOT IN ('KSL 11',
                                                 'KM 3',
                                                 'KM 4',
                                                 'KM 5',
                                                 'KM 6',
                  ...
                  FROM (SELECT Z.CONTRACT,
                          ...
                          FROM (WITH A
                                ...
                                SELECT M.*,
                                  ...
                                  FROM MV_ORGANIZATION M
                                       LEFT OUTER JOIN A
                                          ON (    M.CONTRACT = A.CONTRACT
                                              AND M.BUSINESS_CLUSTER =
                                                     A.BUSINESS_CLUSTER
                          ...
                          SELECT Z.CONTRACT,
                                 ...
                                 SUM (Z.SERVICED) AS SERVICED,
                                      DECODE (
                                         ...
                                         0, 1,
                                         ROUND (
                                              DECODE (SUM (Z.ACHIEVED),
                                                      0, SUM (Z.SERVICED),
                                                      SUM (Z.ACHIEVED))
                                            / DECODE (SUM (Z.SERVICED),
                                                      0, 1,
                                                      SUM (Z.SERVICED)),
                                            3))
                                    * 100
                                    ...
                                    FROM MV_ORGANIZATION M
                                         LEFT OUTER JOIN A
                                            ON (    M.CONTRACT = A.CONTRACT
                                                ...
                                                AND M.CLOSED_DATE = A.CLOSED_DATE))
                                 Z
                           WHERE --Z.INCIDENT_TYPE IN ('User Service Request','User Service Restoration')
                                       ...
                                       COALESCE (
                                          (  SELECT COUNT (*)
                                              ...
                                              WHERE     X.INCIDENT_TYPE IN
                                                    ...
                                                    AND X.BUSINESS_CLUSTER =
                                                           P_BUSINESS_CLUSTER
                                                    AND X.SERVICE = M.SERVICE
                                           GROUP BY X.CONTRACT,
                                                    X.BUSINESS_CLUSTER,
                                                    X.SERVICE,
                                                    TO_CHAR (x."CLOSED_DATE",
                                                             'YYYY_MM') --HAVING X.CONTRACT = 'CAEE II' AND X.BUSINESS_CLUSTER = M.BUSINESS_CLUSTER AND X.SERVICE=M.SERVICE
                                                                       ),
                                          0)
                                          AS ACHIEVED,
                                       COALESCE (A.SERVICED, 0) AS SERVICED,
                                       'KSL 11' AS KSL
                                  FROM (SELECT DISTINCT N.CONTRACT,
                                                        N.BUSINESS_CLUSTER,
                                                        n.service,
                                                        N.CLOSED_DATE
                                          FROM MV_ORGANIZATION N) M
                                       LEFT JOIN
                                       (  SELECT CONTRACT,
                                                 BUSINESS_CLUSTER,
                                                 SERVICE,
                                       ...
                                       (  SELECT CONTRACT,
                                        ...
                                        GROUP BY CONTRACT,
                                          ...
                                          ON (    M.CONTRACT = B.contract
                                              AND M.business_cluster =
                                                     B.business_cluster
                                              AND m.service = b.service
                                              AND m.closed_date = B.closed_date))
                               Z
                         WHERE     Z.BUSINESS_CLUSTER = P_BUSINESS_CLUSTER
                               AND Z.CLOSED_DATE = P_CLOSED_DATE
   ...
   /*FUNCTION ks_2 (P_BUSINESS_CLUSTER VARCHAR2, P_CLOSED_DATE VARCHAR2) RETURN T_TABLE_OVER PIPELINED
     IS
     rec T_COL_OVER;
     BEGIN
   ...
   FUNCTION Overview_T (P_BUSINESS_CLUSTER VARCHAR2, P_CLOSED_DATE VARCHAR2)
      RETURN T_TABLE_OVER
      PIPELINED
   IS
      rec   T_COL_OVER;
      ...
      FOR rec
       ...
       --
                       ...
                       SELECT Z.CONTRACT,
                              ...
                              -- Z.SERVICE,
                              Z.CLOSED_DATE,
                              'Infrastructure Event and Infrastructure Restoration'
                                 AS INCIDENT_TYPE,
                              Z.PRIORITY,
                              SUM (Z.MISSED) AS MISSED,
                              SUM (Z.ACHIEVED) AS ACHIEVED,
                     ...
                     SELECT z.CONTRACT,
                            ...
                            z.serviced,
                                 (ROUND (
                                       (z.serviced - z.achieved - z.MISSED)
                                     / DECODE (z.serviced, 0, 1, z.serviced),
                                     3))
                               * 100
                            || '% '
                               AS "%",
                       ...
                       FROM (SELECT M.CONTRACT,
                                    ...
                                    --  M.SERVICE,
                                    M.CLOSED_DATE,
                                    'User Service Restoration and User Service Request'
                                       AS INCIDENT_TYPE,
                                    'All' AS PRIORITY,
                                    COALESCE (B.MISSED, 0) AS MISSED,
                                    COALESCE (
                                       (  SELECT COUNT (*)
                                        ...
                                        GROUP BY X.CONTRACT,
                                                 ...
                                                 -- X.SERVICE,
                                                 TO_CHAR (x."CLOSED_DATE",
                                                          'YYYY_MM') --HAVING X.CONTRACT = 'CAEE II' AND X.BUSINESS_CLUSTER = M.BUSINESS_CLUSTER
                                                                    ),
                                       0)
                                       AS ACHIEVED,
                                    COALESCE (A.SERVICED, 0) AS SERVICED,
                     ...
                     UNION ALL
                       SELECT A.contract,
                              ...
                              SUM (a.serviced) AS serviced,
                                   ROUND (
                                        (SUM (a.serviced) - SUM (a.missed))
                                      / DECODE (SUM (a.serviced),
                                                0, 1,
                                                SUM (a.serviced)),
                                      3)
                                 * 100
                     ...
                     GROUP BY A.contract, a.BUSINESS_CLUSTER, a.closed_date
                     --Z.INCIDENT_TYPE IN ('User Service Request','User Service Restoration')
                     ...
                     SELECT m.contract,
                            ...
                            COALESCE (v."%", 'n/a %') AS "%",
                            CASE m.PRIORITY
                               WHEN 'Critical' THEN 'KM 4'
                               WHEN 'High' THEN 'KM 5'
                               WHEN 'Medium' THEN 'KM 6'
                               WHEN 'Low' THEN 'KM 7'
                            END
                               AS "KLS"
                       FROM (SELECT DISTINCT n.contract,
                     ...
                     SELECT m.contract,
                            ...
                            COALESCE (V.SERVICED, 0) AS serviced,
                            COALESCE (
                               DECODE (v.serviced, 0, 'n/a %', v.serviced),
                               'n/a %')
                               AS "%",
                            'KM 9' AS "KLS"
                       FROM (SELECT DISTINCT
                            ...
                            (  SELECT v.CONTRACT,
                                      v.BUSINESS_CLUSTER,
                                      v.COMPLETED_DATE_YEAR_MONTH,
                                      --v.class,
                                      COUNT (v.CHANGE_ID) AS serviced
                                 FROM v_km_9 v
                                WHERE     v.class IN ('Emergency', 'Expedited')
                                      AND v.status IN ('Closed', 'Completed')
                                      AND v.COC_STATUS = 'Current'
                                      AND v.ITOM_SCOPE = 'Yes'
                             GROUP BY v.CONTRACT,
                                      v.BUSINESS_CLUSTER,
                                      v.COMPLETED_DATE_YEAR_MONTH   --,v.class
                                                                 ) v
                               ON (    m.contract = v.contract
                                   AND m.business_cluster =
                                          v.business_cluster
                                   AND m.closed_date =
                                          v.COMPLETED_DATE_YEAR_MONTH -- AND m.priority=v.priority
                                                                     )
                      WHERE     M.BUSINESS_CLUSTER = P_BUSINESS_CLUSTER
                            AND M.CLOSED_DATE = P_CLOSED_DATE
                     --km 9
                     ...
                     SELECT CONTRACT,
                       ...
                       FROM TABLE (
                               P_KSL.KSL_11_T (
                                  P_BUSINESS_CLUSTER   => P_BUSINESS_CLUSTER,
                            ...
                            COALESCE (
                               (SELECT COUNT (*)
                                 ...
                                 WHERE     x.SERVICE = 'caee-im:global'
                                       AND TO_CHAR (x.closed_date, 'YYYY_MM') =
                                              P_CLOSED_DATE
                                       AND x.status = 'Closed'),
                               0)
                               AS MISSED,
                            (  COALESCE (V.SERVICED, 0)
                             - COALESCE (
                                  (SELECT COUNT (*)
                                     FROM v_incident_all_data x
                                    WHERE     x.SERVICE = 'caee-im:global'
                                          AND TO_CHAR (x.closed_date,
                                                       'YYYY_MM') =
                                                 P_CLOSED_DATE
                                          AND x.status = 'Closed'),
                                  0))
                     ...
                     SELECT M.CONTRACT,
                            ...
                            COALESCE (V.SERVICED, 0) AS SERVICED,
                                 ROUND (
                                    DECODE (
                                       V.SERVICED,
                                       0, 1,
                                       (V.SERVICED - V.MISSED) / V.SERVICED),
                                    3)
                               * 100
                            || '% '
                               AS "%",
                       ...
                       FROM (SELECT DISTINCT
                            ...
                            (  SELECT V.CONTRACT,
                                      ...
                                      COUNT (*) AS SERVICED,
                                      SUM (
                                         DECODE (COORDINATED_BY_PROVIDER,
                                                 'Yes', 1,
                                                 0))
                                         AS ACHIEVED,
                                      SUM (
   ...
   FUNCTION Overview_R (PP_BUSINESS_CLUSTER    VARCHAR2,
                        PP_CLOSED_DATE         VARCHAR2)
      RETURN T_TABLE_OVER_R
      PIPELINED
   IS
      rec   T_COL_OVER_r;
   BEGIN
        SELECT contract,
          ...
          INTO rec
          FROM TABLE (
                  P_KSL.OVERVIEW_T (P_BUSINESS_CLUSTER   => PP_BUSINESS_CLUSTER,
                                    P_CLOSED_DATE        => PP_CLOSED_DATE))
